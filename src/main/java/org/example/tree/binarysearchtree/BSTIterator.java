...
public class BSTIterator {
    ...
    public BSTIterator(TreeNode root) {
        stack = new Stack<>();
        pushLeft(root);// 初始化时将根节点及其所有左子树节点加入栈
    }
    ...
    private void pushLeft(TreeNode node) {
        while (node != null) {
            ...
            node = node.left;// 继续处理左子节点
        }
    }

    /**
     * 返回下一个最小值，并将栈中当前节点的右子树的左子树节点压入栈中
     *
     * @return 中序遍历下一个节点的值
     */
    public int next() {
        TreeNode node = stack.pop(); // 弹出栈顶元素（最小的节点）
        pushLeft(node.right);// 将当前节点的右子树的所有左子节点压入栈
        return node.val;// 返回栈顶节点的值
    }

    /**
     * 判断是否有下一个节点
     *
     * @return 如果栈非空，说明还有下一个节点
     */
